...
- ~~Worker応答の`requestId`照合は未実装~~ → **v3.9.13で実装済み**。`onWorkerMessage()`冒頭で`INIT_OK`以外は`requestId`不一致時に即returnして無視する
...
| **v3.9.2〜v3.9.17** | **オフライン対応・6-max/9-max・ヒートマップ精緻化・Hero Hand機能を追加した長期セッション（詳細はSTATE_MANAGEMENT.mdのUI改修履歴を参照）。** 主な変更点のみ要約: Blob URL経由のWorker起動でfile://でも動作するオフライン対応。6-max/9-max対応とTIGHT/NORMAL/LOOSEレンジ幅トグル。◥バッジの重み付けバグ修正（スーテッド25%/ペア50%）、ブロードウェイ枠線の廃止、コンボ内訳ツールチップ（対角塗り＋スート記号+勝率%）、区分的ガンマ+boardHazard強化によるカラーマッピング精緻化、requestId照合によるstale response対策。STREET-box FLUSH/STRAIGHT%は「盤面が化ける確率」から「レンジ側の統計」に定義変更。**HERO HAND機能を新設**（独立パネル・独立モバイルタブ、メイン計算経路から完全に分離した設計方針。役名/勝率目安/将来性/hero固有フラッシュ・ストレート完成率に加え、FOLD/CALL/RAISE x2/RAISE x3+の4択判定と対3bet時のFOLD/CALL/4BETの3択判定をヒューリスティックで表示。ソルバー解ではない旨を明記）。フォールバック計算エンジンは`fallback-reference.js`に退避し、失敗時は正直に「ENGINE ERROR」を表示する方針に変更 |
| **v3.9.18〜v3.9.19** | **Worker初期化失敗時の可視性改善とHERO HAND判定の精緻化。** `finalizeBrainReady()`が成功/失敗を問わず600ms後にローディング画面を強制的に隠していたバグを修正し、失敗時（`ENGINE ERROR`）はエラー内容が画面に残り続けるように変更（DevToolsを開かなくても原因が読めるように、ヘッダーバッジも誤解を招く`BRAIN:READY`から正しく`BRAIN:ERROR`（赤）に変更）。HERO HANDの判定表示を、単一の判定を強調する方式から、FOLD/CALL/RAISE x2/RAISE x3+（および3bet時のFOLD/CALL/4BET）それぞれの割合を常に合計100%のバーで示す頻度表示に変更（`heroDecisionDistribution()`、combined値からのガウス的距離で算出）。あわせてプリフロップ（ボード未入力）でもChen percentileベースの相対的な強さを表示できるようにHERO HANDの対象範囲を拡張 |
